...
export default class {
  ...
  handleEditTicket(e, bill, bills) {
    ...
    const ticketIsSelected = $(`#open-bill${bill.id}`).data('selected');

    if (ticketIsSelected) {
        $(`#open-bill${bill.id}`).removeData('selected')
      ...
      $('.vertical-navbar').css({ height: '120vh' })
        ...
        bills.forEach(b => {
            ...
            $(`#open-bill${b.id}`).data('selected', false)
        })

        $(`#open-bill${bill.id}`).data('selected', true)
  ...
  handleShowTickets(e, bills, index) {
    ...
    const isOpenArrow = arrow.data('open');
